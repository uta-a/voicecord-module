...
現在のソースと成果物に基づく進捗: [PROGRESS.md](PROGRESS.md)。2026-09-15 に確認。
...
## 残作業の優先順（2026-09-15 更新）

現在の完了状態: M2・M3・M4 は完了。M4.5・M5・M6 は実装済み部分の受け入れが残る。
下の履歴にある古い未チェック項目だけで、実装の有無を判断しない。
実機操作はヘッドレスの Playwright MCP + Canary の CDP。Stable は M7 まで触らない。
...
3. [ ] M4.5: Vencord 共存を新 UI で再検証する。
   Canary のみで連鎖し、採取役・接ぎ木・再生・テーマ・Discord 本体が動くことを確認する。
   Vencord の掴み方と衝突しないことを確認し、検証後は元の構成へ戻す。
...
5. [ ] M1: Discord 回帰確認の残りを完了する。
   ...
   ユーザー操作でのメッセージ送信、OS 通知バナー・通知音は未確認。
   受信側は一般 VC に参加済み。画面共有の受信表示は確認済み。メッセージ送信と通知の実操作は明示許可後に行う。
   2026-09-15: 一般テキストチャンネルへ `VoiceCord 回帰確認` を送信し、Canary 側で表示を確認。OS 通知バナー・通知音は未確認。
...
7. [ ] M6: 最新の portable exe をビルドし、配布版から受け入れる。
   2026-09-15: build.mjs と electron-builder が完了し、`dist/VoiceCord-0.1.0-portable.exe` を更新。unpacked resources に engine、patcher、preload、harvest、hook、ui と payload/node_modules を確認。Canary への exe 経由受け入れは未実施。
   UI 修正を含む最新版で exe を再生成。frida と payload の同梱を確認する。
   exe から Canary への適用・解除・更新後の再適用・復旧が通ることを確認する。
   2026-09-15: portable exe を起動して Canary 1.0.1173 と Vencord 連鎖を検出。実物が有効でも古い state.json だけで更新警告が出る不具合を修正し、再生成した exe で警告消失を確認。適用・解除の往復は未実施。
   AV 案内・パス表示・失敗理由も確認する。古い exe の存在だけで完了にしない。
8. [ ] M6: ドキュメントとタスクの古い記述を整理する。
   実装済みと実機確認待ちを区別し、注入先・実測レート・ABI・起動方法・ヘッドレス接続を揃える。
   旧 README の凍結注記は記録上完了。旧解析文書を変更しない確定方針を維持する。
   DevTools オプトインの未実装項目は受け入れに必要か整理してから扱う。
9. [ ] M7: Canary の受け入れ完了後に Stable の適用範囲を確認する。
   読み取りプローブで発火関数・バッファ・レートを実測してから注入を検証する。
   実際の float / int16 経路、校正・受信音声・Vencord 共存・停止復帰を確認する。
